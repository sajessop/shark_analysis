...
setwd(
  'M:/Agency Data/Draft Publications/Braccini/2019-20_Parks Australia Project/Fieldwork/Data'
)
TEPS <-
  read_excel("TEPS interactions.xlsx", sheet = "Sheet1", skip = 1)
...
if (Event.Mes.data.dump == 'Jack')
{
  ...
  Video.net.interaction <-
    read_excel(file.name.GN, sheet = "Interactions")
  ...
  Video.longline.interaction <-
    read_excel(file.name.LL, sheet = "Interactions")
  Video.longline.maxN <- read_excel(file.name.LL, sheet = "MaxN")
  Video.longline.obs <-
    read_excel(file.name.LL, sheet = "Observations")
  
  #habitat
  #note: CATAMI code used to determine habitat types
  file.name.habitat = "Gillnet_longline habitat.xlsx"
  Video.habitat <-
    read_excel(file.name.habitat, sheet = "gillnet habitat")
  Video.habitat.LL <-
    read_excel(file.name.habitat, sheet = "longline habitat")
  
  
  #6. PA - deck camera 1 (points to measuring board)
  file.name.habitat.deck = "Deck 1 habitat and fish_6_12_2021.xlsx"
  Video.habitat.deck <-
    read_excel(file.name.habitat.deck, sheet = "Habitat")
  Video.camera1.deck <-
    read_excel(file.name.habitat.deck, sheet = "Deck 1 fish landed")
  Video.camera1.deck_extra.records <-
    read_excel(file.name.habitat.deck, sheet = "extra records")
  
  
  #7. PA - deck camera 2 (points to roller)
  file.name.camera2.deck = "Deck 2_6_12_2021.xlsx"
  Video.camera2.deck <-
    read_excel(file.name.camera2.deck, sheet = "Deck 2")
  Video.camera2.deck_observations <-
    read_excel(file.name.camera2.deck, sheet = "Other observations")
  
  
  #8. PA - subsurface camera
  file.name.subsurface = "SubSurface_7_12_2021.xlsx"
  Video.subsurface <-
    read_excel(file.name.subsurface, sheet = "ALL Dot Point Measurements")
    ...
    Video.net.interaction[[i]] = dummy.GN[[i]] %>%
      ...
      mutate(
        ...
        Alt.species = case_when(
          Escape == "7 legged startfish" ~ "seven legged starfish",
          Escape %in% c(
            "bait schiool",
            "school",
            "School",
          ...
          Escape %in% c(
            "cuttle fish",
            "CUTTLEFISH",
            "cuttlefish-attrached to camera"
          ) ~ "cuttlefish",
          Escape %in% c("unidentifiable school", "UNKNONW FISH") ~
            "unknown fish",
          Escape %in% c("australian fur seal", "sealion", "Australian Seal Lion") ~
            "sea lion",
          Escape %in% c("commernat", "comorant") ~
            "commorant",
          Escape %in% c(
            "too dark",
            "end-no haul",
            ...
            "too dark form here",
            "Gets Dark",
            "too dark after this point",
            "to dark after this point",
          ...
          Escape %in% c("no fish", "No Fish Seen") ~
            "no fish",
          Escape %in% c(
            "30 sec",
            "3 min",
    ...
    Video.net.obs[[i]] = dummy.GN[[i]] %>%
      ...
      mutate(
        ...
        observation = case_when(
          observation == "7 legged startfish" ~ "seven legged startfish",
          grepl("\\d", observation) ~ '',
          Got.dark == 'TRUE' ~ '',
          observation %in% c("squid", "SQUID") ~
            "Squid",
          observation %in% c(
            "cuttle fish",
            "CUTTLEFISH",
          ...
          observation %in% c("unidentifiable school", "UNKNONW FISH") ~
            "unknown fish",
          observation %in% c("australian fur seal", "sealion") ~
            "sea lion",
    ...
    Video.longline.interaction[[i]] = dummy.LL[[i]] %>%
      ...
      mutate(
        ...
        Alt.species = case_when(
          ...
          Escape %in% c(
            "unidentifiable school",
            "UNKNONW FISH",
            "fish sp unsure",
            "unknown  school"
          ) ~ "unknown fish",
          Escape %in% c("australian fur seal", "sealion") ~
            "sea lion",
          Escape %in% c("commernat", "comorant") ~
            "commorant",
          Escape %in% c(
          ...
          Escape %in% c("no fish", "No Fish Seen", "NO FISH SEEN", "no fish seen") ~
            "no fish",
          Escape %in% c(
            "BIRD DIVING FOR BAIT",
            "bird",
            "birds feeding at surface",
            "sear water",
            "bird-diving for bait"
          ) ~ "bird",
          Escape %in% c(
...
if (User == "Matias")
{
  source(handl_OneDrive(
    "Analyses/SOURCE_SCRIPTS/Git_Population.dynamics/fn.fig.R"
  ))
...
distance.roller.spreader = mean(
  c(
    distance.roller.spreader.Anthony,
    distance.roller.spreader.Tim,
    distance.roller.spreader.Nils
  ),
  na.rm = T
)
...
Length.weight.main = data.frame(
  common_name = Main.species,
  a_weight = c(
    1.23e-5,
    1.63e-5,
    1.36e-6,
  ...
  b_FL.to.TL = c(1.9133, 7.818082, 3.8499, 6.182718,
                 rep(NA, 4))
) #Norris et al 2016 & Smallwood et al 2018 for scalefish

Length.weight.discards = data.frame(
  common_name = c(
    "Buffalo bream",
    "Dusky morwong",
    "Port Jackson",
  ...
  a_FL.to.TL = rep(NA, 7),
  b_FL.to.TL = rep(NA, 7)
)  #Simpfendorfer & McAuley 2003 & Coulson pers com
...
Additional.length.weight.economics =
  data.frame(
    ...
    a_FL.to.TL = c(rep(NA, 21), rep(
      mean(Length.weight.main$a_FL.to.TL[1:4]), 13
    )),
    b_FL.to.TL = c(rep(NA, 21), rep(
      mean(Length.weight.main$b_FL.to.TL[1:4]), 13
    ))
  )


Length.weight = rbind(Length.weight.main,
                      Length.weight.discards,
                      Additional.length.weight.economics)
...
TEPS.codes = c(TEPS_Shark.rays,
               TEPS_marine.mam,
               TEPS_seabirds,
               TEPS_reptiles)
TEPS.names = data.frame(
  Name = c(
    'Grey nurse shark',
    'White shark',
    'Smooth stingray',
    'Black stingray',
    'Oversized dusky shark',
    'Shearwaters',
    'Cormorants',
...
suppressWarnings({
  All.species.names = All.species.names %>%
    mutate(Code = ifelse(
      Taxa == 'Elasmobranch' & !is.na(CAAB_code) & nchar(CAAB_code) == 4,
      as.numeric(paste('3700', CAAB_code, sep = '')),
      ifelse(
        Taxa == 'Elasmobranch' & !is.na(CAAB_code) & nchar(CAAB_code) == 5,
        as.numeric(paste('370', CAAB_code, sep = '')),
        ifelse(Taxa == 'Teleost' &
                 !is.na(CAAB_code), as.numeric(paste(
                   '37', CAAB_code, sep = ''
                 )),
               NA)
      )
...
Hook.combos <- Hook.combos %>%
  mutate(
    hooks.baited = C10.W + C12.W + C14.W + Eb10.W + Eb12.W + Eb14.W +
...
DATA = DATA %>%
  mutate(
    hooktype = case_when(
      hooktype %in% c('Circular', 'Offset circular') ~ 'circular',
...
DATA = DATA %>%
      ...
      left_join(
        Lost.snoods %>%
          ...
          `colnames<-`(paste(colnames(Lost.snoods), '.lost', sep =
                               '')) %>%
          dplyr::select(
            -c(Date.lost, Unknown.lost, 'Total broken.lost', Comments.lost)
          ),
  ...
  mutate(
    Effort = case_when(
      method == "GN" ~ soak.time * net_length,
      method == "LL" ~ soak.time * n.hooks
    ),
    Effort.hook.combo = case_when(
      method == "LL" &
        hooktype == 'circular' &
        hooksize == 10 & wiretrace == 'Yes' ~ soak.time * C10.W,
      method == "LL" &
        hooktype == 'circular' &
        hooksize == 12 & wiretrace == 'Yes' ~ soak.time * C12.W,
      method == "LL" &
        hooktype == 'circular' &
        hooksize == 14 & wiretrace == 'Yes' ~ soak.time * C14.W,
      method == "LL" &
        hooktype == 'circular' &
        hooksize == 10 & wiretrace == 'No' ~ soak.time * C10.M,
      method == "LL" &
        hooktype == 'circular' &
        hooksize == 12 & wiretrace == 'No' ~ soak.time * C12.M,
      method == "LL" &
        hooktype == 'circular' &
        hooksize == 14 & wiretrace == 'No' ~ soak.time * C14.M,
      method == "LL" &
        hooktype == 'Ezb' &
        hooksize == 10 & wiretrace == 'Yes' ~ soak.time * Eb10.W,
      method == "LL" &
        hooktype == 'Ezb' &
        hooksize == 12 & wiretrace == 'Yes' ~ soak.time * Eb12.W,
      method == "LL" &
        hooktype == 'Ezb' &
        hooksize == 14 & wiretrace == 'Yes' ~ soak.time * Eb14.W,
      method == "LL" &
        hooktype == 'Ezb' &
        hooksize == 10 & wiretrace == 'No' ~ soak.time * Eb10.M,
      method == "LL" &
        hooktype == 'Ezb' &
        hooksize == 12 & wiretrace == 'No' ~ soak.time * Eb12.M,
      method == "LL" &
        hooktype == 'Ezb' &
        hooksize == 14 & wiretrace == 'No' ~ soak.time * Eb14.M,
      TRUE ~ NA_real_
    )
  )
...
DATA = DATA %>%
  arrange(date) %>%
  mutate(
    diff_days = difftime(date, lag(date, n = 1), units = "days"),
  ...
  group_by(trip) %>%
  mutate(
    C10.W.infered = ifelse(is.na(C10.W), round(mean(C10.W, na.rm = T)), C10.W),
    C12.W.infered = ifelse(is.na(C12.W), round(mean(C12.W, na.rm =
                                                      T)), C12.W),
    C14.W.infered = ifelse(is.na(C14.W), round(mean(C14.W, na.rm =
                                                      T)), C14.W),
    C10.M.infered = ifelse(is.na(C10.M), round(mean(C10.M, na.rm =
                                                      T)), C10.M),
    C12.M.infered = ifelse(is.na(C12.M), round(mean(C12.M, na.rm =
                                                      T)), C12.M),
    C14.M.infered = ifelse(is.na(C14.M), round(mean(C14.M, na.rm =
                                                      T)), C14.M),
    Eb10.W.infered = ifelse(is.na(Eb10.W), round(mean(Eb10.W, na.rm =
                                                        T)), Eb10.W),
    Eb12.W.infered = ifelse(is.na(Eb12.W), round(mean(Eb12.W, na.rm =
                                                        T)), Eb12.W),
    Eb14.W.infered = ifelse(is.na(Eb14.W), round(mean(Eb14.W, na.rm =
                                                        T)), Eb14.W),
    Eb10.M.infered = ifelse(is.na(Eb10.M), round(mean(Eb10.M, na.rm =
                                                        T)), Eb10.M),
    Eb12.M.infered = ifelse(is.na(Eb12.M), round(mean(Eb12.M, na.rm =
                                                        T)), Eb12.M),
    Eb14.M.infered = ifelse(is.na(Eb14.M), round(mean(Eb14.M, na.rm =
                                                        T)), Eb14.M)
  ) %>%
  ungroup() %>%
  mutate(
    Sum = ifelse(
      method == 'LL',
      C10.W.infered + C12.W.infered + C14.W.infered + Eb10.W.infered + Eb12.W.infered +
        Eb14.W.infered +
        C10.M.infered + C12.M.infered + C14.M.infered + Eb10.M.infered +
        Eb12.M.infered + Eb14.M.infered,
      NA
    ),
    C10.W.infered = ifelse(
      !n.hooks == Sum,
      floor(n.hooks * C10.W.infered / Sum),
      C10.W.infered
    ),
    C12.W.infered = ifelse(
      !n.hooks == Sum,
      floor(n.hooks * C12.W.infered / Sum),
      C12.W.infered
    ),
    C14.W.infered = ifelse(
      !n.hooks == Sum,
      floor(n.hooks * C14.W.infered / Sum),
      C14.W.infered
    ),
    Eb10.W.infered = ifelse(
      !n.hooks == Sum,
      floor(n.hooks * Eb10.W.infered / Sum),
      Eb10.W.infered
    ),
    Eb12.W.infered = ifelse(
      !n.hooks == Sum,
      floor(n.hooks * Eb12.W.infered / Sum),
      Eb12.W.infered
    ),
    Eb14.W.infered = ifelse(
      !n.hooks == Sum,
      floor(n.hooks * Eb14.W.infered / Sum),
      Eb14.W.infered
    ),
    C10.M.infered = ifelse(
      !n.hooks == Sum,
      floor(n.hooks * C10.M.infered / Sum),
      C10.M.infered
    ),
    C12.M.infered = ifelse(
      !n.hooks == Sum,
      floor(n.hooks * C12.M.infered / Sum),
      C12.M.infered
    ),
    C14.M.infered = ifelse(
      !n.hooks == Sum,
      floor(n.hooks * C14.M.infered / Sum),
      C14.M.infered
    ),
    Eb10.M.infered = ifelse(
      !n.hooks == Sum,
      floor(n.hooks * Eb10.M.infered / Sum),
      Eb10.M.infered
    ),
    Eb12.M.infered = ifelse(
      !n.hooks == Sum,
      floor(n.hooks * Eb12.M.infered / Sum),
      Eb12.M.infered
    ),
    Eb14.M.infered = ifelse(
      !n.hooks == Sum,
      floor(n.hooks * Eb14.M.infered / Sum),
      Eb14.M.infered
    )
  ) %>%
  mutate(
  ...
  mutate(
    Effort.wire.infered = case_when(
      method == "LL" &
        wiretrace == 'Yes' ~ soak.time * (
          C10.W.infered + C12.W.infered + C14.W.infered + Eb10.W.infered + Eb12.W.infered +
            Eb14.W.infered
        ),
      method == "LL" &
        wiretrace == 'No' ~ soak.time * (
          C10.M.infered + C12.M.infered + C14.M.infered + Eb10.M.infered + Eb12.M.infered +
            Eb14.M.infered
        ),
      TRUE ~ NA_real_
    ),
    Effort.hooksize.infered = case_when(
      method == "LL" &
        hooksize == 10 ~ soak.time * (C10.W.infered + C10.M.infered + Eb10.W.infered +
                                        Eb10.M.infered),
      method == "LL" &
        hooksize == 12 ~ soak.time * (C12.W.infered + C12.M.infered + Eb12.W.infered +
                                        Eb12.M.infered),
      method == "LL" &
        hooksize == 14 ~ soak.time * (C14.W.infered + C14.M.infered + Eb14.W.infered +
                                        Eb14.M.infered),
      TRUE ~ NA_real_
    ),
    Effort.hooktype.infered = case_when(
      method == "LL" &
        hooktype == 'circular' ~ soak.time * (
          C10.W.infered + C12.W.infered + C14.W.infered + C10.M.infered + C12.M.infered +
            C14.M.infered
        ),
      method == "LL" &
        hooktype == 'Ezb' ~ soak.time * (
          Eb10.W.infered + Eb12.W.infered + Eb14.W.infered + Eb10.M.infered + Eb12.M.infered +
            Eb14.M.infered
        ),
      TRUE ~ NA_real_
    )
...
DATA = DATA %>%
  mutate(
    n.hooks = ifelse(
      method == 'LL' & is.na(n.hooks) & !is.na(hooks.baited),
      hooks.baited,
      n.hooks
    ),
...
DATA = DATA %>%
  group_by(trip) %>%
  mutate(
    soak.time.infRD = ifelse(is.na(soak.time), round(mean(soak.time, na.rm =
                                                            T)), soak.time),
    ...
    Effort.infRD = ifelse(
      ...
      ifelse(
        is.na(Effort) & method == 'GN',
        net_length.infRD * soak.time.infRD,
        Effort
      )
    )
  ) %>%
...
if (explr.dat.entry)
{
  ...
  if (nrow(No.times) > 0)
    write.csv(No.times, 'No.times.csv', row.names = F)
  
  No.position = DATA %>%
    filter(is.na(mid.lat) | is.na(mid.long)) %>%
    distinct(sheet_no, .keep_all = T)
  if (nrow(No.position) > 0)
    write.csv(No.position, 'No.position.csv', row.names = F)
  
  No.species = DATA %>%
    filter(is.na(common_name) | is.na(scientific_name))
  if (nrow(No.species) > 0)
    write.csv(No.species, 'No.species.csv', row.names = F)
  
  
  LL.issues = DATA %>%
    filter(method == "LL") %>%
    filter(is.na(hooktype) |
             is.na(hooksize) | is.na(wiretrace) | n.hooks < 1) %>%
    dplyr::select(sheet_no,
                  ...
                  wiretrace)
  if (nrow(LL.issues) > 0)
    write.csv(LL.issues, 'LL.issues.csv', row.names = F)
  ...
  a = DATA %>%
    ...
    dplyr::select(sheet_no,
                  ...
                  common_name)
  if (nrow(a) > 0)
    write.csv(a, 'GN.with.longline.data_n.hooks_not_NA.csv', row.names = F)
  ...
  GN.issues = DATA %>% filter(method == "GN") %>%
    ...
    distinct(sheet_no, .keep_all = T)
  if (nrow(GN.issues) > 0)
    write.csv(GN.issues, 'GN.issues.csv', row.names = F)
  
  a = DATA %>%
    filter(method == 'LL' & (net_length >= 0 |
                               !is.na(mesh_size))) %>%
    dplyr::select(sheet_no, line_no, net_length, method, mesh_size, common_name)
  if (nrow(a) > 0)
    write.csv(a, 'LL.with.gillnet.data.csv', row.names = F)
  ...
  if (nrow(a) > 0)
    write.csv(a, 'N_hooks.different_to_hooks.deployed.csv', row.names = F)
  
  #retained issues
  dodgy.retained = c("DM.T", "SK.T", "WC", "PJ", "SH", "WW", "SR", "BK", "FR")
  a = DATA %>% filter(species %in% dodgy.retained &
                        retainedflag == 'Yes') %>%
    dplyr::select(sheet_no, line_no, species, retainedflag) %>%
    arrange(sheet_no, line_no)
  if (nrow(a) > 0)
    write.csv(a, 'discards_entered_as_retained.csv', row.names = F)
  
  check = c("BG.T", "QS.T", "PS.T", 'JE.T',
            "TK", "WH", "CP", "BW", "GM")
  a = DATA %>% filter(species %in% check &
                        retainedflag == "No") %>% distinct(sheet_no,
                                                           line_no,
                                                           species,
                                                           retainedflag,
                                                           fl,
  ...
  mtext(
    expression(paste("Latitude (", degree, "S)", sep = "")),
    side = 2,
    line = 1.25,
    las = 3,
    cex = 1.5
  ...
  if (nrow(no.soak) > 0)
    write.csv(no.soak, 'No_soak_time.csv', row.names = F)
  
  no.netlen = DATA %>% filter(method == "GN") %>% filter(is.na(net_length)) %>%
    distinct(sheet_no)
  if (nrow(no.netlen) > 0)
    write.csv(no.netlen, 'No_net_length.csv', row.names = F)
  
  #no number of hook combos
  no.n.huk.com = Hook.combos %>% dplyr::select(
    sheet_no,
    C10.W,
    C12.W,
    C14.W,
  ...
  if (length(not.in.huk.combo) > 0)
    no.n.huk.com = sort(c(no.n.huk.com, not.in.huk.combo))
  write.csv(
    no.n.huk.com,
    "no.number.of.hook.combinations_in_Hook_count_xlsx.csv",
    row.names = F
  )
}
                           ...
                           LGSIZE = 12)
{
  ...
  if (Group == '95')
    what.species.GN = what.species.GN %>% mutate(species2 = ifelse(Cumktch <=
                                                                     0.95, species, 'Other'))
  if (Group == 'Top20')
    what.species.GN = what.species.GN %>% mutate(species2 = ifelse(Row <= 20, species, 'Other'))
  ...
  what.species.LL = d %>%
    ...
    mutate(Row = 1:n(),
           ...
           Cumktch = Cumktch / sum(n))
  if (Group == '95')
    what.species.LL = what.species.LL %>% mutate(species2 = ifelse(Cumktch <=
                                                                     0.95, species, 'Other'))
  if (Group == 'Top20')
    what.species.LL = what.species.LL %>% mutate(species2 = ifelse(Row <= 20, species, 'Other'))
  ...
  if (Transf == 'proportion')
    d[-match(c(Terms, 'ColSum'), names(d))] = d[-match(c(Terms, 'ColSum'), names(d))] /
    d$ColSum
  d = d %>%
    dplyr::select(-ColSum) %>%
    data.frame
  ...
  adon.results <-
    adonis(formula(paste('Community', Show.term, sep = '~')),
           data = d,
           method = "bray",
           perm = 5e3)
  write.csv(
    as.data.frame(adon.results$aov.tab),
    paste(hndl, Permanova.title, '.csv', sep = "")
  )
  ...
  write.csv(
    adonis.pairwise,
    paste(hndl, Permanova.title, '_pairwise.csv', sep = ""),
    row.names = F
  )
  ...
  SIMPER <-
    summary(simper(Community, d %>% pull(Show.term), parallel = 7))
  ...
  write.csv(do.call(rbind, SIMPER.out),
            paste(hndl, Simper.title, '.csv', sep = ""),
            row.names = F)
}
...
{
  if (biascor == "YES")
    #apply bias correction for log transf
  {
    lsm = summary(emmeans(mod, PRED, type = "link")) %>%
      mutate(
        response = exp(emmean) * exp(SE ^ 2 / 2),
        lower.CL = exp(lower.CL) * exp(SE ^ 2 / 2),
        upper.CL = exp(upper.CL) * exp(SE ^ 2 / 2)
      )
  }
  if (biascor == "NO")
    lsm = summary(emmeans(mod, PRED, type = "response"))
...
pred.fun.continuous = function(d, mod, PRED, Formula)
{
  ...
  if (PRED[1] %in% VARS)
  {
    ...
    if (length(PRED) == 1)
    {
      ...
      if (length(NewD) < 50)
        NewD = seq(min(NewD), max(NewD), length.out = 100)
...
plt.map = function(D, Title, Scaler)
{
  p = ggplot(oz_states) +
    ...
    geom_point(
      ...
      size = D$Prop * Scaler,
      colour = grDevices::adjustcolor("steelblue", alpha = 0.6)
    ) +
    geom_sf_text(
      data = sites,
      aes(label = Cityname),
      color = "White",
      fontface = "bold",
      nudge_x = Nudge$nudge_x,
      nudge_y = Nudge$nudge_y
    ) +
...
TEPS.code_contact = data.frame(
  contact.code = c('WWC', 'BFC', 'WER', 'WEN', 'WDNN', 'WDNF', 'WDDN', 'WDDF'),
  contact.code.meaning = c(
    'Wildlife on/in water, contact with vessel',
  ...
  contact.code.to.complete = c(
    'on/in water, contact with vessel',
...
TEPS.code_behav = data.frame(
  sighting.behav = c('INT', 'IRR', 'DES', 'ROW'),
  sighting.behav.meaning = c(
...
TEPS <- TEPS %>%
  ...
  mutate(contact.code = ifelse(contact.code == "WEN/ WWC" &
                                 sheet_no == 'PA0036', NA, contact.code)) %>%
  data.frame %>%
...
TEPS = TEPS %>%
  mutate(
    ...
    SP.group = case_when(
      common.name %in% c("grey nurse shark") ~ "protected sharks",
      common.name %in% c("australian sealion") ~ "marine mammals",
      TRUE ~ "other stuff"
    ),
    common.name = case_when(
      common.name == "australian sealion" ~ "Australian sea-lion",
      TRUE ~ common.name
    ),
...
Video.net.obs = Video.net.obs %>%
  mutate(
    Interaction = case_when(!code %in% c(54079901, 11183901, 11183902) ~ 'Swim Past'),
    Escape = NA,
    Method = "Gillnet",
    Position = "Gillnet",
    Species = tolower(observation)
  )
if (!is.na(match('code', colnames(Video.net.obs))))
  Video.net.obs = Video.net.obs %>% rename(Code = code)
...
Video.longline.obs = Video.longline.obs %>%
  mutate(
    ...
    Interaction = ifelse(
      grepl('birds feeding', Observation),
      paste("feeding from", Method),
      NA
    ),
    Escape = NA,
    Species = tolower(Observation)
...
if (!is.na(match('code', colnames(Video.longline.obs))))
{
  if (!is.na(match('Code', colnames(Video.longline.obs))))
  {
    if (is.na(sum(Video.longline.obs$Code)))
      Video.longline.obs = Video.longline.obs %>% dplyr::select(-Code)
...
Video.net.interaction = Video.net.interaction %>%
  mutate(
    SP.group = case_when(
      Code >= 3.7e7 & Code <= 3.70241e7 ~ "Sharks",
      Code > 3.7025e7 &
        Code <= 3.7041e7 ~ "Rays",
      Code >= 3.7042e7 &
        Code <= 3.7044e7 ~ "Chimaeras",
      Code >= 3.7046e7 &
        Code <= 3.747e7 ~ "Scalefish",
      Code >= 3.747e7 &
        Code <= 4.0e+07 ~ "Turtles",
      Code >= 4.1e+07 &
        Code <= 4.115e+07 ~ "Marine mammals",
      Code >= 4.0e+07 &
        Code < 4.1e+07 ~ "Seabirds",
      Code >= 1.2e7 &
        Code < 3.7e7 ~ "Invertebrates",
      Code >= 1.1e7 &
        Code < 1.2e7 ~ "Rock/reef structure",
      Code >= 5.4e7 &
        Code < 5.49e7 ~ "Macroalgae",
      Code == 10000910 ~ "Sponges"
    ),
    Period = tolower(Period),
    Depth = as.numeric(gsub("[^0-9.-]", "", Depth)),
    OpCode = case_when(
      OpCode == 'Augusta_Gillnet_PA00013_Camera_#S1' ~ 'Augusta_Gillnet_PA0013_Camera_#S1',
      TRUE ~ OpCode
    ),
    ...
    Interaction2 = ifelse(
      Interaction %in% grep(paste(c("Caught", "caught"), collapse = "|"), Interaction, value =
                              TRUE) &
        !is.na(Escape2),
      "Escape",
      Interaction
...
Video.net.interaction = Video.net.interaction %>%
  ...
  mutate(
    ...
    Retain.group =
      ifelse(
        retained == 'Yes' &
          SP.group %in% c('Sharks', 'Rays'),
        "Retained elasmobranch",
        ifelse(
          retained == 'No' &
            SP.group %in% c('Sharks', 'Rays'),
          "Discarded elasmobranch",
          ifelse(
            retained == 'Yes' &
              SP.group %in% c('Scalefish'),
            "Retained scalefish",
            ifelse(
              retained == 'No' &
                SP.group %in% c('Scalefish'),
...
Video.longline.interaction = Video.longline.interaction %>%
  mutate(
    ...
    Depth = as.numeric(gsub("[^0-9.-]", "", Depth)),
    OpCode = case_when(
      OpCode == 'Augusta_longline_PA20024_Camera_#2' ~ 'Augusta_longline_PA0024_Camera_#2',
      TRUE ~ OpCode
    ),
    sheet_no = sapply(strsplit(OpCode, "_"), "[", 3),
    Camera = paste("Camera", sapply(strsplit(OpCode, "_"), "[", 5)),
...
DATA = DATA %>%
  mutate(
    hooktype = capitalize(hooktype),
    wiretrace = tolower(wiretrace),
    wiretrace = ifelse(wiretrace == 'no', "Mono", ifelse(wiretrace ==
                                                           "yes", "Wire", NA)),
...
DATA = DATA %>% mutate(
  dw = ifelse(CAES_Code %in% 35000:39100, pl, NA),
  Size = case_when(
    taxa == 'Elasmobranch' ~ fl,
    taxa == 'Teleost' ~ tl,
...
DATA = DATA %>%
  ...
  mutate(
    tl = case_when(
      is.na(tl) &
        common_name %in% these.sharks ~ a_FL.to.TL * fl +
        b_FL.to.TL,
      TRUE ~ tl
    ),
    tl = ifelse(CAES_Code %in% 35000:39100 & is.na(tl), dw, tl),
    tw = ifelse(!is.na(a_weight) &
                  !is.na(tl), a_weight * tl ^ b_weight, NA),
...
with(subset(N.fshn, zone == "Zone2"),
     table(method, skipper, useNA = 'ifany'))
...
if (do.map)
{
  ...
  library(rgdal)
  SDGDLL_zone1 = readOGR(handl_OneDrive("Data/Mapping/Shark_shape_files/SDGDLL_zone1.shp"),
                         layer = "SDGDLL_zone1")
  SDGDLL_zone2 = readOGR(handl_OneDrive("Data/Mapping/Shark_shape_files/SDGDLL_zone2.shp"),
                         layer = "SDGDLL_zone2")
  WCDGDLL = readOGR(handl_OneDrive("Data/Mapping/Shark_shape_files/WCDGDLL.shp"),
                    layer = "WCDGDLL")
  ...
  text(122,-27,
       ...
       font = 2.5)
  polygon(
    x = c(Long.range, rev(Long.range)),
    y = c(rep(Lat.range[1], 2), rep(Lat.range[2], 2)),
    col = rgb(.1, .6, .1, alpha = .4),
    border = "black"
  )
...
write.csv(
  Tab.n.sp.gear,
  le.paste("Observer/Observed.number.species.by.gear.csv"),
  row.names = F
)
...
write.csv(
  Tab.n.sp.longline,
  le.paste("Observer/Observed.number.species.by.longline.config.csv"),
  row.names = F
)


# 2. Number of individuals caught by snood configuration
fn.fig(le.paste("Observer/Barplot Longline configurations"),
       2400,
       2000)
DATA %>%
  filter(method == "LL") %>%
...
fn.brplt = function(d, Grouped.species, CX.nm)
{
  dd = d %>%
    ...
    mutate(
      new.COMMON_NAME = ifelse(
        COMMON_NAME %in% Grouped.species & Taxa == 'Elasmobranch',
        "Other elasmobranchs",
        ifelse(
          COMMON_NAME %in% Grouped.species & Taxa == 'Teleost',
          "Other scalefish",
          COMMON_NAME
        )
      )
  ...
  n.other.elasmo = d %>% filter(COMMON_NAME %in% Grouped.species &
                                  Taxa == 'Elasmobranch') %>% pull(COMMON_NAME)
  n.other.elasmo = length(unique(n.other.elasmo))
  n.other.scalies = d %>% filter(COMMON_NAME %in% Grouped.species &
                                   Taxa == 'Teleost') %>% pull(COMMON_NAME)
  n.other.scalies = length(unique(n.other.scalies))
  
  dd = dd %>%
    mutate(
      colr = ifelse(Taxa == 'Elasmobranch', "steelblue", "firebrick"),
      colr = ifelse(
        new.COMMON_NAME %in% c("Other elasmobranchs", "Other scalefish"),
        "forestgreen",
        colr
      ),
      new.COMMON_NAME = ifelse(
        new.COMMON_NAME == "Other elasmobranchs",
        paste("Other elasmos (n=", n.other.elasmo, " species)", sep =
                ""),
        ifelse(
          new.COMMON_NAME == "Other scalefish",
...
#Overall GN vs LL
fn.fig(le.paste("Observer/catch_comp/Catch.comp_GN_LL_overall"),
       2600,
       1600)
par(
  mfcol = c(1, 2),
...
mtext(paste(
  "Gillnet (",
  length(unique(subset(DATA, method == 'GN')$sheet_no)),
  " shots, ",
  sum(subset(DATA, method == 'GN')$number),
  " individuals)",
  sep = ""
),
3,
...
mtext(paste(
  "Longline (",
  length(unique(subset(DATA, method == 'LL')$sheet_no)),
  " shots, ",
  sum(subset(DATA, method == 'LL')$number),
  " individuals)",
  sep = ""
),
3,
...
fn.fig(le.paste("Observer/catch_comp/Catch.comp_GN_LL_by.zone"),
       2600,
       2000)
smart.par(
  length(dis.zoun) * 2,
...
for (z in 1:length(dis.zoun))
{
  ...
  if (z == 1)
    mtext("Gillnet", 3, cex = .9)
  LGN = paste(
    dis.zoun[z],
    " (",
    length(unique(
      subset(DATA, method == 'GN' &
               zone == dis.zoun[z])$sheet_no
    )),
    " shots, ",
    sum(subset(DATA, method == 'GN' &
                 zone == dis.zoun[z])$number),
    " individuals)",
    sep = ""
  )
  ...
  if (z == 1)
    mtext("Longline", 3, cex = .9)
  LGN = paste(
    dis.zoun[z],
    " (",
    length(unique(
      subset(DATA, method == 'LL' &
               zone == dis.zoun[z])$sheet_no
    )),
    " shots, ",
    sum(subset(DATA, method == 'LL' &
                 zone == dis.zoun[z])$number),
    " individuals)",
    sep = ""
  )
...
for (i in 1:length(LL.configs))
{
  ...
  fn.brplt(d = d,
           Grouped.species = Grouped.species.LL,
           CX.nm = .7)
  mtext(paste(LL.configs[i], " (", nrow(d), " individuals)", sep = ""),
        3,
        cex = .6)
  rm(d)
}
...
for (i in 1:length(nn))
{
  ...
  fn.brplt(d = d,
           Grouped.species = Grouped.species.LL,
           CX.nm = .7)
  mtext(paste(nn[i], " (", nrow(d), " individuals)", sep = ""), 3, cex =
          .6)
  rm(d)
}
mtext(
...
fn.fig(le.paste("Observer/size/Size.density.dist.main.sp_GN_LL"),
       2400,
       1600)
dumi %>%
...
fn.fig(
  le.paste(
    "Observer/size/Size.density.dist.main.sp_by_LL.configuration"
  ),
  2400,
  1600
)
...
for (o in 1:length(Overlap.res))
{
  x <- list(
    GN = DATA %>%
      filter(common_name == Tabl.sizes[o] &
               !is.na(Size) & method == 'GN') %>%
      pull(Size),
    LL = DATA %>%
      filter(common_name == Tabl.sizes[o] &
               !is.na(Size) & method == 'LL') %>%
      pull(Size)
  )
  out <- overlap(x)
  #final.plot(x,out$OV)
  Overlap.res[[o]] = data.frame(
    common_name = Tabl.sizes[o],
    overlap = out$OV,
    N.GN = length(x$GN),
    N.LL = length(x$LL)
  )
...
fn.fig(le.paste("Observer/size/Size.density.dist.overlap_GN_LL"),
       2700,
       1600)
DATA %>%
  ...
  mutate(
    method = ifelse(method == "GN", "Gillnet",
                    ifelse(method == "LL", "Longline",
                           NA)),
    common_name = ifelse(
      common_name == "West Australian dhufish",
      "WA dhufish",
      common_name
    ),
    LBL = paste(
...
fn.kolmo = function(GN, LL)
  Kolmo = ks.test(GN, LL)
for (s in 1:length(KS.result))
{
  KS.result[[s]] = fn.kolmo(
    GN = DATA %>%
      filter(common_name == Tabl.sizes[s] &
               !is.na(Size) & method == 'GN') %>%
      pull(Size),
    LL = DATA %>%
      filter(common_name == Tabl.sizes[s] &
...
Tab.Kolmo = data.frame(
  Species = unlist(Tab.Kolmo$Species),
...
write.csv(Tab.Kolmo,
          le.paste("Observer/size/Size.Kolmo_Smirnov.csv"),
          row.names = F)
...
fn.glm.size = function(d, Formula, Pred, Anova.title)
{
  ...
  #export anova table
  ft <- as_flextable(mod)
  save_as_image(ft, path = le.paste(paste(
    "Observer/size/ANOVA_", Anova.title, ".png", sep = ''
  )))
  rm(ft)
  ...
  } else
  {
    ...
    if (length(a) > 0)
    {
      ...
      {
        p[[ss]] = pred.fun(mod = mod,
                           biascor = "NO",
                           PRED = Pred[str_detect(names(a)[ss], Pred)]) %>%
          mutate(species = NM)
      }
      return(p)
    }
...
for (m in 1:length(this.sp.sizes))
{
  ...
  if (sum(Tab$n) > 1)
  {
    Store.zone.method[[m]] = fn.glm.size(
      d = D,
      Formula = formula(Size ~ method +
                          zone),
      Pred = 'method',
      Anova.title = paste("Method", NM, sep =
                            '_')
    )
  }
  
}
fn.fig(le.paste("Observer/size/Standardized_Size_Method"),
       2400,
       2400)
out = fn.barplot.cpue(
  d = do.call(rbind, Store.zone.method) %>%
    mutate(
      species = factor(species, levels = this.sp.sizes),
      x = method,
      y = emmean,
      fill = method,
      facet = species
...
for (m in 1:length(this.sp.sizes))
{
  ...
  D = DATA %>%
    ...
    filter(!is.na(hooktype) &
             !is.na(wiretrace) & !is.na(hooksize)) %>%
    mutate(
  ...
  if (sum(Tab$n) > 1)
  {
    a = fn.glm.size(
      d = D,
      Formula = formula(Size ~ hooktype + hooksize + wiretrace +
                          zone),
...
#Hook size
fn.fig(le.paste("Observer/size/Standardized_Size_Hook.size"),
       2000,
       1400)
out = fn.barplot.cpue(
  d = fun.list.out(LIS = Store, what = 'hooksize') %>%
    mutate(
      species = factor(species, levels = this.sp.sizes),
      x = hooksize,
      y = emmean,
...
#Hook type
fn.fig(le.paste("Observer/size/Standardized_Size_Hook.type"),
       1400,
       2400)
out = fn.barplot.cpue(
  d = fun.list.out(LIS = Store, what = 'hooktype') %>%
    mutate(
      species = factor(species, levels = this.sp.sizes),
      x = hooktype,
      y = emmean,
...
{
  fn.fig(le.paste("Observer/size/Standardized_Size_Snood.type"),
         2000,
         2000)
  out = fn.barplot.cpue(
    d = fun.list.out(LIS = Store, what = 'wiretrace') %>%
      mutate(
        species = factor(species, levels = this.sp.sizes),
        x = wiretrace,
        y = emmean,
...
if (do.events)
{
  ...
  rbind(
    Video.longline.interaction %>% dplyr::select(Method, Interaction, Number, SP.group, Species),
  ...
  ggsave(
    le.paste(
      "Video/underwater/Interactions_number.events_sqrt.transf_by.group.tiff"
    ),
    width = 12,
    height = 10,
    compression = "lzw"
  ...
  ggsave(
    le.paste(
      "Video/underwater/Interactions_number.events_sqrt.transf.tiff"
    ),
    width = 12,
    height = 8,
    compression = "lzw"
...
ggsave(
  le.paste(
    "Video/underwater/Interactions_number.individuals_sqrt.transf_by.group.tiff"
  ),
  width = 15,
  height = 10,
  compression = "lzw"
...
ggsave(
  le.paste(
    "Video/underwater/Interactions_number.individuals_sqrt.transf.tiff"
  ),
  width = 13,
  height = 8,
  compression = "lzw"
...
d = rbind(
  Video.longline.interaction %>% dplyr::select(Method, Interaction, Number, Code),
...
ggsave(
  le.paste(
    "Video/underwater/Interactions_number.individuals_main.target.tiff"
  ),
  width = 12,
  height = 10,
  compression = "lzw"
...
add.effort = rbind(
  Video.longline.interaction %>% dplyr::select(sheet_no, Method, Camera),
...
Abbey.data.chapter.1_species = rbind(
  Video.longline.interaction %>% dplyr::select(sheet_no, Method, Interaction, Number, Code, SP.group),
...
Abbey.data.chapter.1_SP.group = rbind(
  Video.longline.interaction %>% dplyr::select(sheet_no, Method, Interaction, Number, SP.group),
  ...
  data.frame %>%
  left_join(add.effort, by = c('sheet_no', 'Method'))
Abbey.data.chapter.1_Retain.group = rbind(
  Video.longline.interaction %>% dplyr::select(sheet_no, Method, Interaction, Number, Retain.group),
  ...
  left_join(add.effort, by = c('sheet_no', 'Method'))
write.csv(
  Abbey.data.chapter.1_species,
  handl_OneDrive(
    'Analyses/Parks Australia/outputs/Data for Abbey/Abbey.data.chapter.1_species.csv'
  ),
  row.names = F
)
write.csv(
  Abbey.data.chapter.1_SP.group,
  handl_OneDrive(
    'Analyses/Parks Australia/outputs/Data for Abbey/Abbey.data.chapter.1_SP.group.csv'
  ),
  row.names = F
)
write.csv(
  Abbey.data.chapter.1_Retain.group,
  handl_OneDrive(
    'Analyses/Parks Australia/outputs/Data for Abbey/Abbey.data.chapter.1_Retain.group.csv'
  ),
  row.names = F
)
...
if (do.loli)
{
  ...
  df = rbind(
    Video.longline.interaction %>% dplyr::select(Method, Interaction, Number),
  ...
  ggplot(df) +
    ...
    coord_flip()
  ggsave(
    le.paste("Video/underwater/Interactions_lollipop.tiff"),
    width = 12,
    height = 10,
    compression = "lzw"
  )
...
{
  d = rbind(
    Video.longline.interaction %>% dplyr::select(Method, Interaction, Number, SP.group, Genus, Species, Code),
  ...
  write.csv(
    TAB,
    le.paste(
      "Video/underwater/Table_number.interactions.by.species.csv"
    ),
    row.names = F
  )
  ...
  if (trans == 'proportion')
    Community.GN = Community.GN / rowSums(Community.GN)
  if (trans == 'squareroot')
    Community.GN = Community.GN ^ (1 / 2)
  if (trans == '4throot')
    Community.GN = Community.GN ^ (1 / 4)
  
  Community.LL <<- TAB.LL[-id]
  if (trans == 'proportion')
    Community.LL = Community.LL / rowSums(Community.LL)
  if (trans == 'squareroot')
    Community.LL = Community.LL ^ (1 / 2)
  if (trans == '4throot')
    Community.LL = Community.LL ^ (1 / 4)
  ...
  simper.GN = simper.fn(
    ...
    tav = TAB.GN,
    SUBT = "Gillnet",
    Sig.pair = adonis.pairwise.GN %>% filter(P < 0.05) %>%
      pull(Pairs)
  )
  simper.LL = NULL
  if (nrow(adonis.pairwise.LL %>% filter(P < 0.05)) > 0)
  {
    simper.LL = simper.fn(
      ...
      tav = TAB.LL,
      SUBT = "Longline",
      Sig.pair = adonis.pairwise.LL %>% filter(P < 0.05) %>%
        pull(Pairs)
    )
  }
...
ggarrange(
  plotlist = list(
    out.under.n$p.mdsGN + rremove("xlab") + rremove("ylab"),
...
ggsave(
  le.paste("Video/underwater/interactions_MDS.tiff"),
  width = 5,
  height = 8,
  compression = "lzw"
)
...
write.csv(
  out.under.n$Permanova.GN,
  le.paste("Video/underwater/interactions_Permanova.GN.csv")
)
write.csv(
  out.under.n$Permanova.LL,
  le.paste("Video/underwater/interactions_Permanova.LL.csv")
)
write.csv(
  out.under.n$adonis.pairwise.GN,
  le.paste("Video/underwater/interactions_Permanova_pairwise.GN.csv"),
  row.names = F
)
write.csv(
  out.under.n$adonis.pairwise.LL,
  le.paste("Video/underwater/interactions_Permanova_pairwise.LL.csv"),
  row.names = F
)

plist = list(out.under.n$simper.GN)
if (!is.null(out.under.n$simper.LL))
  plist = list(out.under.n$simper.GN, out.under.n$simper.LL) #%>%discard(is.null)
...
ggsave(
  le.paste("Video/underwater/interactions_simper.tiff"),
  width = 6,
  height = 8,
  compression = "lzw"
)
...
Hab.cols = c('lightskyblue',
             'red4',
             'seagreen',
             'orange3',
             'tomato4',
             'khaki')
names(Hab.cols) = c('Hydroids',
                    'Macroalgae',
                    'Seagrasses',
                    'Sponges',
                    'Stony corals',
...
Out.damg_under_LL = fn.habitat.damg.sub(d = Video.habitat.LL %>%
                                        ...
                                        TITL = "Longline")
ggarrange(
  Out.damg_under_GN$p1,
  Out.damg_under_LL$p1,
  Out.damg_under_GN$p2,
  Out.damg_under_LL$p2,
  Out.damg_under_GN$p3,
  Out.damg_under_LL$p3,
  ncol = 2,
  nrow = 3
)
ggsave(
  le.paste("Video/underwater/Habitats_coarse.records.tiff"),
  width = 10,
  height = 10,
  compression = "lzw"
)
...
Net.damage.underwater = Video.net.interaction %>%
  mutate(Habitat.damage = ifelse(
    SP.group %in% c("Macro algae", "Rock/reef structure", "Sponges"),
    "Damage",
    "No damage"
  )) %>%
...
text = paste(
  Damage.events %>% filter(Habitat.damage == 'Damage') %>% pull(n),
...
ggsave(
  le.paste("Video/underwater/Habitat.damage_GN.tiff"),
  width = 6,
  height = 10,
  compression = "lzw"
)
...
{
  Damage.events = rbind(Damage.events, data.frame(Habitat.damage = "Damage", n =
                                                    0))
}
Total.observed.metres = length(unique(Video.longline.interaction$OpCode)) *
  metres.observed
text = paste(
  Damage.events %>% filter(Habitat.damage == 'Damage') %>% pull(n),
  "habitat damage events in\n",
...
ggplot() +
  ...
  theme(plot.title = element_text(size = 20, face = "bold"))
ggsave(
  le.paste("Video/underwater/Habitat.damage_LL.tiff"),
  width = 6,
  height = 10,
  compression = "lzw"
)
...
Video.habitat.deck = Video.habitat.deck %>%
  ...
  mutate(
    Period = tolower(Period),
    SHEET_NO = case_when(
      Period == 'gillnet' ~ str_remove(word(DIPRD.code, 1, sep = "\\/"), 'GN'),
  ...
  mutate(
    net_length = net_length * 1000,
    #add net length in m
    Species = ifelse(Species == "Radiata", "Macroalgae", Species),
    #group Eklonia with macroalgae
    Period = ifelse(
      SHEET_NO %in% c("PA0129", "PA0094", "PA0084"),
      'longline',
      Period
    )
  )   #wrongfully allocated method
...
fn.habitat.damg.deck = function(Gear)
{
  ...
  fn.hab.explr = function(d.no_zeros, d)
  {
    ...
    mtext("Without 0s",
          ...
          col = "steelblue",
          cex = 1.5)
    fn.hist(d$Coral, "Coral")
    fn.hist(d.no_zeros$Coral, "Coral")
    fn.hist(d$Rock, "Rock")
    fn.hist(d.no_zeros$Rock, "Rock")
    fn.hist(d$Seagrass, "Seagrass")
    fn.hist(d.no_zeros$Seagrass, "Seagrass")
    fn.hist(d$Sponge, "Sponge")
    fn.hist(d.no_zeros$Sponge, "Sponge")
  ...
  pdf(le.paste(
    paste(
      "Video/deck.cameras/Habitat.damage_explore_",
      Gear,
      ".pdf",
      sep = ''
    )
  ...
  p2 = d %>%
    ...
    geom_text(
      x = 3.5,
      aes(y = labelPosition, label = paste(Percent, "%")),
      size = 8,
      check_overlap = TRUE
    ) +
    ...
    ggtitle(
      ...
      subtitle = paste(
        ...
        distance.roller.spreader,
        ' m of net) with damage',
        sep = ''
      )
    ) +
    theme_void() +
  ...
  p3 = d.no.zeros %>%
    ...
    mutate(
      label = paste(round(100 * Percent / sum(Percent)), '%', sep = ''),
      ymax = cumsum(Percent),
      ymin = c(0, head(ymax, n = -1)),
      labelPosition = (ymax + ymin) / 2
    ) %>%
    ...
    geom_text(
      x = 3.5,
      aes(y = labelPosition, label = label, angle = 0),
      size = 5,
      check_overlap = TRUE
    ) +
...
ggarrange(Out.damg_GN$p2,
          Out.damg_GN$p3,
          Out.damg_GN$p4,
          ncol = 1,
          nrow = 3)
ggsave(
  le.paste("Video/deck.cameras/Habitat.interactions.tiff"),
  width = 6,
  height = 10,
  compression = "lzw"
)
...
Video.camera2.deck = Video.camera2.deck %>%
  ...
  mutate(
    ...
    DPIRD.code = ifelse(
      DPIRD.code == 'GNPA003/LLPA0004',
      'GNPA0003/LLPA0004',
      ifelse(
        DPIRD.code == 'GNPA006/LLPA0005',
        'GNPA0006/LLPA0005',
        DPIRD.code
      )
    ),
    SHEET_NO = case_when(
      Period == 'gillnet' ~ str_remove(word(DPIRD.code, 1, sep = "\\/"), 'GN'),
...
Video.camera2.deck_observations = Video.camera2.deck_observations %>%
  ...
  mutate(
    ...
    Code = ifelse(
      grepl('sea bird', comment) & is.na(Code),
      40000000,
      ifelse(grepl('shear water', comment) &
               Code == 40048000, 40041050,
             Code)
    ),
    SHEET_NO = case_when(
      Period == 'gillnet' ~ str_remove(word(DPIRD.code, 1, sep = "\\/"), 'GN'),
    ...
    Activity = case_when(
      grepl("feeding", comment) ~ paste("feeding from", Period),
      is.na(Activity) &
        grepl("shear water", tolower(comment)) ~ 'resting',
...
Video.subsurface.comments = Video.subsurface.comments %>%
  ...
  mutate(
    ...
    Code = ifelse(grepl('shearwater', comment) &
                    Code == 40128000, 40041050, Code),
    SHEET_NO = case_when(
      Period == 'gillnet' ~ str_remove(word(DPIRD.code, 1, sep = "\\/"), 'GN'),
      Period == 'longline' ~ str_remove(word(DPIRD.code, 2, sep = "\\/"), 'LL')
    )
...
fn.plt.dropouts = function(d, TITLE, LegPos, var, MinN)
{
  ...
  these.drop.out.sp = as.numeric(names(these.drop.out.sp[these.drop.out.sp >
                                                           MinN]))
  ...
  d = d %>%
    ...
    left_join(All.species.names %>% dplyr::select(COMMON_NAME, Code), by =
                "Code") %>%
    mutate(COMMON_NAME = factor(COMMON_NAME, levels = names(sort(
      these.drop.out.sp
    ))))
...
ggarrange(p1,
          ...
          heights = c(1.5, 1))
ggsave(
  le.paste("Video/deck.cameras/Drop.out.events.tiff"),
  width = 10,
  height = 12,
  compression = "lzw"
)
...
fn.fig(le.paste("Video/deck.cameras/Drop.out.events_predictions_deckcam2"),
       2000,
       2000)
...
fun.table.ret.disc = function(d)
{
  TAB = d %>%
    ...
    mutate(
      Percent.drop.out.gillnet = round(
        100 * Drop.out.Gillnet / (Retained.Gillnet + Drop.out.Gillnet),
        1
      ),
      Percent.drop.out.longline = round(
        100 * Drop.out.Longline / (Retained.Longline + Drop.out.Longline),
        1
      )
    ) %>%
    arrange(COMMON_NAME) %>%
    relocate(
      ...
      Retained.Gillnet,
      Drop.out.Gillnet,
      Percent.drop.out.gillnet,
      Retained.Longline,
      Drop.out.Longline,
      Percent.drop.out.longline
    ) %>%
    mutate_at(vars(Percent.drop.out.longline), ~ replace(., is.nan(.), "")) %>%
    mutate_at(vars(Percent.drop.out.gillnet), ~ replace(., is.nan(.), "")) %>%
    mutate(
      Drop.out.Gillnet = ifelse(Percent.drop.out.gillnet == '', '', Drop.out.Gillnet),
...
write.csv(
  fun.table.ret.disc(d = Video.camera2.deck),
  le.paste("Video/deck.cameras/Drop.out.events_table_camera2.csv"),
  row.names = F
)

write.csv(
  fun.table.ret.disc(d = Video.subsurface),
  le.paste("Video/deck.cameras/Drop.out.events_table_subsurface.csv"),
  row.names = F
)

#1.4 Drop outs fate
p = fn.plt.dropouts(
  d = Video.subsurface %>% filter(!dropout == 'No'),
  TITLE = "Subsurface camera",
  LegPos = "top",
...
ggsave(
  le.paste("Video/deck.cameras/Drop.out.events_fate_subsurface.tiff"),
  width = 10,
  height = 8,
  compression = "lzw"
)
...
p1 = fn.plt.dropouts(
  d = Video.subsurface %>% filter(!dropout == 'No') %>%
    mutate(
      Gaffed = ifelse(dropout == "Yes", "Lost", dropout),
...
p2 = fn.plt.dropouts(
  d = Video.camera2.deck %>%
    mutate(
      Gaffed = case_when(
        dropout == 'Yes' & gaffed %in% c('No') ~  'Lost',
        dropout == 'Yes' &
          gaffed %in% c('Yes') ~  'Gaffed',
        TRUE ~ gaffed
      )
    ) %>%
...
ggsave(
  le.paste("Video/deck.cameras/Drop.out.events_gaffing.tiff"),
  width = 10,
  height = 8,
  compression = "lzw"
)
...
Data_water.column = Video.camera2.deck %>%
  ...
  mutate(
    Code = ifelse(Code == 37018001, 37018003, Code),
...
ggsave(
  le.paste("Video/deck.cameras/Weight_float_species.events.tiff"),
  width = 10,
  height = 8,
  compression = "lzw"
)
...
Mod <-
  glm(cbind(float, weight) ~ COMMON_NAME,
      data = glm.dat,
      family = "binomial")
...
p = fn.barplot.cpue(
  d = Pred %>%
    left_join(data.frame(
      COMMON_NAME = names(Col.vec), Col = Col.vec
    ),
    by = 'COMMON_NAME') %>%
    mutate(
      Group = Col,
...
fn.fig(le.paste("Video/deck.cameras/Weight_float_species_predictions"),
       2400,
       2400)
p + scale_fill_manual(values = c("firebrick", "steelblue")) +
  theme(axis.text.x = element_text(
...
ft <- as_flextable(Mod)
save_as_image(ft,
              path = le.paste("Video/deck.cameras/anovas/Weight_float_species.png"))
...
All.underwater.shts = rbind(
  ...
  mutate(
    Effort = case_when(
      Method == 'Longline' ~ N.cameras * soak.time * hooks.observed,
      Method == 'Gillnet' ~ N.cameras * soak.time *
        metres.observed
    )
  )
...
Rate.under_bait.loss = Rate.under_bait.loss %>%
  left_join(
    All.underwater.shts %>%
      ...
      dplyr::select(sheet_no, N.cameras),
    by = 'sheet_no'
  ) %>%
  ...
  mutate(
    Effort = case_when(
      Method == 'Longline' ~ soak.time * hooks.observed * N.cameras,
      Method == 'Gillnet' ~ soak.time * metres.observed *
        N.cameras
    )
  )

Bait.loss.rate_under = unique(
  rbind(
    Rate.under_bait.loss %>%
      dplyr::select(Method, sheet_no, n, Effort),
    All.underwater.shts %>%     #add observed 0 records
      filter(
        Method == 'Longline' &
          !sheet_no %in% Rate.under_bait.loss$sheet_no
      ) %>%
      mutate(n = 0) %>%
      dplyr::select(Method, sheet_no, n, Effort)
  ) %>%
    ungroup() %>%
    mutate(Rate = sum(n) / sum(Effort, na.rm =
                                 T)) %>%
    pull(Rate)
)
...
Rate.under = rbind(
  ...
  mutate(
    ...
    Method = capitalize(Method),
    Interaction = ifelse(
      Interaction %in% c("Caught while predating", "Predated on"),
      "Depredation",
      Interaction
    )
  )
...
Tab.out = data.frame(
  ...
  Method = c("Longline",
             ...
             "Gillnet", "Longline"),
  Rate = c(
    Bait.loss.rate_under,
    Drepedation.rate_under %>% filter(Method == "Gillnet") %>%
      pull(Rate),
    Drepedation.rate_under %>% filter(Method == "Longline") %>%
      pull(Rate),
    Drop.out.rate_under %>% filter(Method == "Gillnet") %>%
      pull(Rate),
    Drop.out.rate_under %>% filter(Method == "Longline") %>%
      pull(Rate)
  ),
  Units = c(
    "Number of lost baits per 1 hook hour",
    "Number of depredated individuals per 1 m hour",
    "Number of depredated individuals per 1 hook hour",
    "Number of dropouts per 1 m hour",
    "Number of dropouts per 1 hook hour"
  )
) %>%
...
write.csv(
  Tab.out %>%
    ...
    dplyr::select(-Event),
  le.paste("Video/underwater/Rate_Drop.out.csv"),
  row.names = F
)
write.csv(
  Tab.out %>%
    ...
    dplyr::select(-Event),
  le.paste("Video/underwater/Rate_Drepedation.csv"),
  row.names = F
)
write.csv(
  Tab.out %>%
    ...
    dplyr::select(-Event),
  le.paste("Video/underwater/Rate_Bait.loss.csv"),
  row.names = F
)
rm(Tab.out)
...
All.shts = DATA %>%
  ...
  mutate(
    method = ifelse(method == "GN", "Gillnet", ifelse(method == "LL", "Longline", NA)),
  ...
  group_by(method, sheet_no) %>%
  summarise(
    net_length = max(net_length, na.rm = T),
...
Drop.out.rate_deck2 = fn.rates_sub_deck2(d = Video.camera2.deck %>% rename(sheet_no =
                                                                             SHEET_NO),
                                         MinN = Min.N.drop.out)
...
write.csv(
  Drop.out.rate_deck2,
  le.paste("Video/deck.cameras/Rate_Drop.out_deck2.csv"),
  row.names = F
)


Drop.out.rate_subsurface = fn.rates_sub_deck2(
  d = Video.subsurface %>%
    rename(sheet_no = SHEET_NO) %>%
    mutate(dropout = ifelse(dropout ==
                              'Gaffed', 'Yes', dropout)),
  MinN = Min.N.drop.out
)
...
write.csv(
  Drop.out.rate_subsurface,
  le.paste("Video/deck.cameras/Rate_Drop.out_subsurface.csv"),
  row.names = F
)
...
Video.camera1.deck = Video.camera1.deck %>%
  ...
  mutate(
    Period = ifelse(
      is.na(Period) & DIPRD.code == 'GNPA003/LLPA0004',
      'gillnet',
      Period
    ),
    DIPRD.code = ifelse(
      DIPRD.code == 'GNPA003/LLPA0004',
      'GNPA0003/LLPA0004',
      DIPRD.code
    ),
    Period = tolower(Period),
    Period = ifelse(
      Period %in% c('longlien', 'll', 'longlinr'),
      'longline',
...
Video.camera1 = rbind(
  ...
  Video.camera1.deck %>%
    mutate(Code = ifelse(
      Genus == "Kyphosus" & Species == "spp", '37361903', Code
    )) %>%
    dplyr::select(DIPRD.code, Code, Period, number, condition)
) %>%
  data.frame %>%
  ...
  mutate(
    Data.set = "camera",
    Period = ifelse(
      Period == 'gillnet' & method == 'LL',
      'longline',
      ifelse(Period == 'longline' &
               method == 'GN', 'gillnet', Period)
    )
  )

#1. Export data for Abbey
Abbey.data.chapter.2 = rbind(
  Video.camera1 %>%
    dplyr::rename(Number = number) %>%
    mutate(Method = capitalize(Period)) %>%
    filter(Code >= 3.7e7 &
             Code <= 3.747e7) %>%  #compare only fish and sharks
...
Abbey.data.chapter.2_Retain.group = Abbey.data.chapter.2 %>%
  ...
  left_join(add.effort, by = c('sheet_no', 'Method'))
write.csv(
  Abbey.data.chapter.2_species,
  handl_OneDrive(
    'Analyses/Parks Australia/outputs/Data for Abbey/Abbey.data.chapter.2_species.csv'
  ),
  row.names = F
)
write.csv(
  Abbey.data.chapter.2_SP.group,
  handl_OneDrive(
    'Analyses/Parks Australia/outputs/Data for Abbey/Abbey.data.chapter.2_SP.group.csv'
  ),
  row.names = F
)
write.csv(
  Abbey.data.chapter.2_Retain.group,
  handl_OneDrive(
    'Analyses/Parks Australia/outputs/Data for Abbey/Abbey.data.chapter.2_Retain.group.csv'
  ),
  row.names = F
)
...
EM.vs.OM.DATA = DATA %>%
  mutate(
    COMMON_NAME = case_when(
      grepl(paste(c(
        "whaler", "dusky shark"
      ), collapse = '|'), tolower(COMMON_NAME)) ~ 'Whalers',
      grepl(paste(
        c("blacktip sharks", "Common blacktip"), collapse = '|'
      ), tolower(COMMON_NAME)) ~ 'Blacktip sharks',
      grepl('wobbegong', tolower(COMMON_NAME)) ~
        'Wobbegongs',
      grepl('hammerhead', tolower(COMMON_NAME)) ~
        'Hammerheads',
      grepl('stingray', tolower(COMMON_NAME)) ~
        'Stingrays',
      grepl('angel', tolower(COMMON_NAME)) ~ 'Angel sharks',
      grepl('catshark', tolower(COMMON_NAME)) ~
        'Catsharks',
      grepl('gurnard', tolower(COMMON_NAME)) ~
        'Gurnards',
      grepl('jacket', tolower(COMMON_NAME)) ~ 'Leatherjackets',
      grepl('guitarfish', tolower(COMMON_NAME)) ~
        'Guitarfish & shovelnose rays',
      grepl(paste(
        c("gummy shark", "whiskery shark"), collapse = '|'
      ), tolower(COMMON_NAME)) ~ "Gummy/Whiskery sharks",
      TRUE ~ COMMON_NAME
    )
  )
...
TAB.disc = TAB %>%
  filter(retainedflag == 'No' &
           !COMMON_NAME %in% TAB.comm$COMMON_NAME) %>%
  arrange(-n) %>%
  mutate(Cum = cumsum(n),
         Percent = 100 * Cum / sum(n))
...
fn.obs_cam.barplot = function(n.shots, Cam, Obs, LGN, TITL)
{
  ...
  d = rbind(Cam, Obs) %>%
    mutate(Method.hour = ifelse(
      method == 'Longline',
      paste(method, LGN.LL, sep = ''),
...
p1 = fn.obs_cam.barplot(
  n.shots = EM.vs.OM.DATA %>%
    filter(sheet_no %in% unique(Video.camera1$sheet_no)) %>%
  ...
  Obs = EM.vs.OM.DATA %>%
    filter(
      sheet_no %in% unique(Video.camera1$sheet_no) &
...
ggsave(
  le.paste("Camera_v_Observer/Barplot.tiff"),
  width = 12,
  height = 12,
  compression = "lzw"
)
...
little.fn = function(d, Method, group, Not)
{
  ...
  if (group == '95')
  {
    what.species = n %>% mutate(species2 = ifelse(Cumktch <= 0.95, COMMON_NAME, 'Other')) %>%
      pull(species2)
  }
  if (group == 'Top20')
  {
    what.species = n %>% mutate(Row = 1:n(),
                                species2 = ifelse(Row <= 20, COMMON_NAME, 'Other')) %>%
      pull(species2)
  }
  ...
  dd[-match(c(Not, 'ColSum'), names(dd))] = dd[-match(c(Not, 'ColSum'), names(dd))] /
    dd$ColSum
  dd = dd %>%
    dplyr::select(-ColSum) %>%
    data.frame
...
fn.mod = function(dd, SBTL)
{
  ...
  p = dd %>%
    ...
    geom_line(aes(Observer, Observer),
              linetype = 2,
              color = 'black') +
...
lolipop = function(x)
{
  p = x %>%
    ...
    geom_errorbar(
      aes(ymin = xmin, ymax = xmax, x = COMMON_NAME),
      width = .125,
      show.legend = F
    ) +
    geom_point(
      aes(x = COMMON_NAME, y = Mean, fill = method),
      pch = 21,
      size = 4,
      position = position_dodge(width = 0)
    ) +
...
fn.compare.obs.cam = function(CAM, OBS, GROUP, Terms, Minobs.per)
{
  ...
  if (do.cpue)
  {
    ...
    mod.GN <-
      glm(
        log.cpue ~ Platform * COMMON_NAME,
        family = "gaussian",
        data = d %>% filter(method == "GN")
      )
    mod.LL <-
      glm(
        log.cpue ~ Platform * COMMON_NAME,
        family = "gaussian",
        data = d %>% filter(method == "LL")
      )
  }
  zero.ktch.sht = d %>%
    group_by(sheet_no) %>%
    ...
    pull(sheet_no)
  d1 = d
  if (length(zero.ktch.sht) > 0)
    d1 = d1 %>% filter(!sheet_no %in% zero.ktch.sht)
  d1 = d1 %>%
    spread(Platform, n, fill = 0) %>%
    filter(!is.na(method)) %>%
    mutate(method = factor(method))
  ...
  #Permanova
  adon.results <-
    adonis(formula(paste('Community', Terms, sep = '~')),
           data = dd,
           method = "bray",
           perm = 5e3)
...
#export data issues
write.csv(Out$out.for.Jack,
          le.paste("Camera_v_Observer/out.for.Jack.csv"),
          row.names = F)

#export Raw data comparison
Out$p.ind
ggsave(
  le.paste("Camera_v_Observer/raw.tiff"),
  width = 12,
  height = 10,
  compression = "lzw"
)
...
Out$p.sp
ggsave(
  le.paste("Camera_v_Observer/raw_species.tiff"),
  width = 12,
  height = 10,
  compression = "lzw"
)
...
save_as_image(ft,
              path = le.paste("Camera_v_Observer/GLM_mod_commercial.species.png"))
ft <- as_flextable(Out$mod_discarded.species$mod)
save_as_image(ft,
              path = le.paste("Camera_v_Observer/GLM_mod_discarded.species.png"))

fig = ggarrange(
  Out$mod_all.species$p + rremove("xlab") + rremove("ylab"),
...
annotate_figure(
  ...
  left = text_grob(
    "Number of individuals recorded by camera",
    size = 18,
    rot = 90
  )
)
ggsave(
  le.paste("Camera_v_Observer/GLM_preds.tiff"),
  width = 8,
  height = 12,
  compression = "lzw"
)
...
Out$Per.dif
ggsave(
  le.paste("Camera_v_Observer/Percentage.difference.tiff"),
  width = 10,
  height = 12,
  compression = "lzw"
)
...
ggsave(
  le.paste("Camera_v_Observer/MDS.tiff"),
  width = 8,
  height = 12,
  compression = "lzw"
)
...
#export Permanovas
write.csv(Out$adon.GN,
          le.paste("Camera_v_Observer/Permanova_GN.csv"),
          row.names = T)
write.csv(Out$adon.LL,
          le.paste("Camera_v_Observer/Permanova_LL.csv"),
          row.names = T)
...
Video.camera2 = Video.camera2.deck %>%
  mutate(Code = ifelse(Genus == "Kyphosus" &
                         Species == "spp", '37361903', Code)) %>%
...
p1 = fn.obs_cam.barplot(
  ...
  Obs = EM.vs.OM.DATA %>%
    filter(
      sheet_no %in% unique(Video.camera2$sheet_no) &
        COMMON_NAME %in% This.sp
    ) %>%
    mutate(method = ifelse(
      method == "GN", "Gillnet",
...
annotate_figure(fig, bottom = text_grob("Number of individuals", size = 18))
ggsave(
  le.paste("Camera2_v_Observer/Barplot.tiff"),
  width = 12,
  height = 12,
  compression = "lzw"
)
...
#export data issues
write.csv(Out$out.for.Jack,
          le.paste("Camera2_v_Observer/out.for.Jack.csv"),
          row.names = F)

#export Raw data comparison
Out$p.ind
ggsave(
  le.paste("Camera2_v_Observer/raw.tiff"),
  width = 12,
  height = 10,
  compression = "lzw"
)
...
Out$p.sp
ggsave(
  le.paste("Camera2_v_Observer/raw_species.tiff"),
  width = 12,
  height = 10,
  compression = "lzw"
)
...
ft <- as_flextable(Out$mod_commercial.species$mod)
save_as_image(ft,
              path = le.paste("Camera2_v_Observer/GLM_mod_commercial.species.png"))
ft <- as_flextable(Out$mod_discarded.species$mod)
save_as_image(ft,
              path = le.paste("Camera2_v_Observer/GLM_mod_discarded.species.png"))
...
ggsave(
  le.paste("Camera2_v_Observer/GLM_preds.tiff"),
  width = 8,
  height = 12,
  compression = "lzw"
)
...
Out$Per.dif
ggsave(
  le.paste("Camera2_v_Observer/Percentage.difference.tiff"),
  width = 10,
  height = 12,
  compression = "lzw"
)
...
ggsave(
  le.paste("Camera2_v_Observer/MDS.tiff"),
  width = 8,
  height = 12,
  compression = "lzw"
)
...
#export Permanovas
write.csv(Out$adon.GN,
          le.paste("Camera2_v_Observer/Permanova_GN.csv"),
          row.names = T)
write.csv(Out$adon.LL,
          le.paste("Camera2_v_Observer/Permanova_LL.csv"),
          row.names = T)
...
write.csv(
  Tab.gilled.bagged,
  le.paste("Video/deck.cameras/Tab.gilled.bagged.csv"),
  row.names = F
)
...
fn.explr.Abbey.1(
  d = Abbey.data.chapter.1_species %>% dplyr::rename(Eff = n.cameras),
  vars = fn.get.top.1(d = Abbey.data.chapter.1_species, N =
                        250)
)
fn.explr.Abbey.1(
  d = Abbey.data.chapter.1_SP.group %>% dplyr::rename(Eff = n.cameras),
  vars = fn.get.top.1(d = Abbey.data.chapter.1_SP.group, N =
                        30)
)
fn.explr.Abbey.2(
  d = Abbey.data.chapter.2_species %>% dplyr::rename(Eff = Fishing.effort),
  vars = fn.get.top.2(d = Abbey.data.chapter.2_species, N =
                        100)
)
fn.explr.Abbey.2(
  d = Abbey.data.chapter.2_SP.group %>% dplyr::rename(Eff = Fishing.effort),
  vars = fn.get.top.2(d = Abbey.data.chapter.2_SP.group, N =
                        30)
)
dev.off()
...
fn.TEPS.barplot = function(n.shots,
                           d,
                           hours.LL,
                           hours.GN,
                           ...
                           TITL)
{
  ...
  if (!is.null(TITL))
    p = p + ggtitle(TITL)
  if (show.all.levels)
    p = p + scale_fill_manual(values = TEPS.cols)
  if (!show.all.levels)
  {
...
p1 = fn.TEPS.barplot(
  n.shots = rbind(
    ...
    spread(Period, n),
  d = rbind(
    Video.longline.interaction %>% dplyr::select(Method, Interaction, Number, SP.group, Species, Code),
    Video.net.interaction %>% dplyr::select(Method, Interaction, Number, SP.group, Species, Code)
  ) %>%
    mutate(Number = 1,
           Method = capitalize(Method)) %>%
    filter(SP.group %in% TEP.groups |
             Code %in% TEPS_Shark.rays) %>%
    filter(!Species == 'birds feeding at surface') %>%
...
p2 = fn.TEPS.barplot(
  ...
  d = rbind(
    Video.subsurface.comments %>%
      ...
      mutate(
        Method = capitalize(tolower(Period)),
        Number = 1,
    ...
    Video.subsurface %>%
      ...
      mutate(
        ...
        dummy = case_when(Drop.out == "Yes" ~
                            "Drop out",
                          Drop.out == "No" ~ "Caught"),
        Interaction = case_when(
          dummy == "Drop out" ~ paste(dummy, " (", Dropout.condition, ")", sep = ''),
          dummy == "Caught" ~
            dummy
        )
      ) %>%
...
p3 = fn.TEPS.barplot(
  ...
  d = Video.camera1 %>%
    filter(Code %in% TEPS.codes) %>%
    mutate(
      Interaction = paste("Caught (", condition, ")", sep = ''),
      Code = as.numeric(Code)
    ) %>%
    left_join(TEPS.names, by = "Code") %>%
    mutate(Number = 1,
...
p4 = fn.TEPS.barplot(
  n.shots = rbind(
    Video.camera2.deck_observations %>% distinct(Period, SHEET_NO),
  ...
  d = rbind(
    ...
    Video.camera2.deck %>%
      ...
      mutate(Activity = ifelse(
        dropout == 'Yes' & gaffed == "No",
        "Drop out",
        ifelse(dropout == 'Yes' &
                 gaffed == "Yes", "Drop out and gaffed",
               NA)
      )) %>%
      dplyr::select(Period, Genus, Species, Code, Activity)
  ) %>%
    ...
    mutate(Interaction = case_when(
      Activity %in% c("Feeding from gillnet",
                      "Feeding from longline") ~
        "Feeding",
      TRUE ~ Activity
    )) %>%
    mutate(Name = factor(Name, levels = TEPS.names$Name)),
...
d.teps = TEPS %>%
  ...
  mutate(
    ...
    contact.code.to.complete = case_when(
      contact.code.to.complete == 'Diving for but not feeding from net/longline' ~
        'Diving',
      contact.code.to.complete == 'Entangled/hooked' ~
        'Caught',
      contact.code.to.complete == 'Feeding from net/longline' ~
        'Feeding',
      contact.code == 'WDNN; WDDN' ~
        'Feeding',
      TRUE ~ contact.code.to.complete
    ),
    contact.code.to.complete = ifelse(
      contact.code.to.complete == 'Caught',
...
d.data = rbind(
  DATA %>%
    filter(grepl(
      paste(tolower(
        c('bird', 'sea lion', 'Smooth', TEPS.names$Name)
      ), collapse = '|'), tolower(comments)
    )),
  DATA %>%
    filter(common_name %in% TEPS.names$Name),
  DATA %>%
    filter(common_name == "Dusky shark" &
             tl > 190) %>%    #190 is tl corresponding to 70 cm idl
...
ggsave(
  le.paste("TEPS/Interactions.tiff"),
  width = 15.2,
  height = 13,
  compression = "lzw"
)
...
if (do.inter.for.each.shot)
{
  fn.tep.barplot = function(d, all.gn.shots, all.ll.shots)
  {
    ...
    d.nets = d.nets %>%
      mutate(
        contact.code.to.complete = str_remove_all(contact.code.to.complete, Drop),
        con.code.sp = paste(capitalize(common.name), contact.code.to.complete),
        con.code.sp = ifelse(con.code.sp == "NA NA", NA, con.code.sp)
      )
    ...
    ggsave(
      le.paste("TEPS/Numbers.interactions.by.gear.tiff"),
      width = 12,
      height = 10,
      compression = "lzw"
    )
...
if (check.ASL)
{
  #underwater
  ASL.uw = rbind(
    Video.longline.interaction %>% dplyr::select(OpCode, Method, Interaction, Number, SP.group, Species, Code),
