...
#### METHOD COLUMN NEEDS FIXING LOTS OF VARIATION THAT SHOULDNT EXIST
# mutate(Video.net.interaction$Method="Gillnet",
# Video.longline.interaction$Method="Longline",
# ect)
```
## Checks
```{r}
...
```{r}
  setwd(
    'C:/Users/S.Jesso/OneDrive - Department of Primary Industries And Regional Development/Final_EMobs/Deck/OutputDeck2/19-01-23'
  )
dummy.d2 <- list()  
deck2filenames <- dir(pattern="*.csv")
  ...
  # dummy.d2 <- lapply(deck2filenames, read.csv, skip = 4)
```
...
Deck.2.fish = Deck.2.obs = vector('list', length(dummy.d2))
 for (i in 1:length(dummy.d2))
  {
    dummy.d2[[i]] <- DeckColumns(dummy.d2[[i]])
 }
```
